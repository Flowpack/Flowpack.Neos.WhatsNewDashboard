...
// env API to select system under test (SUT) (neos8 | neos9) and flow context for the configuration to be used (default, with source url, etc.)
...
export default defineConfig({
  ...
  webServer: {
    // No `--build`: the image is built once by CI (or by `make setup-sut` locally)
    // and re-used here.
